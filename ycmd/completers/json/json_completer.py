...
def ShouldEnableJSONCompleter():
  if not os.path.exists( PATH_TO_SERVER ):
    LOGGER.info( "Not using JSON completer: not installed" )
    return False

  if not PATH_TO_NODE:
    LOGGER.warning( "Not using JSON completer: can't find node" )
    return False

  LOGGER.info( "JSON completer is ready to use" )
  return True


class JSONCompleter( slsc.SimpleLSPCompleter ):
  def __init__( self, user_options ):
    super( JSONCompleter, self ).__init__( user_options )
